...
class ExportadorDados {
    ...
    processarLinhaCSV(registro) {
        try {
            ...
            const campos = [
                ...
                registro.saida || '',
                registro.pausa || 0,
                this.formatarHorasCSV(calculo.horasTrabalhadas),
                this.formatarHorasCSV(calculo.bancoHoras),
                this.limparTipoPlantao(calculo.tipoPlantao || this.obterTipoTexto(registro)),
                this.formatarNumeroCSV(calculo.valorHora),
                this.formatarNumeroCSV(calculo.valorBase),
                this.formatarNumeroCSV(calculo.horasExtrasRemuneradas),
                this.formatarNumeroCSV(calculo.valorBonus),
                this.formatarNumeroCSV(calculo.valorTotal),
    ...
    gerarResumoCSV(registros) {
        try {
            ...
            resumo += `Saldo Total do Banco,${totais.totalBancoHoras}\n`;
            resumo += `Total Bônus 90%,${totais.totalHorasExtras}\n`;
            resumo += `Valor Total a Receber,${this.formatarNumeroCSV(totais.totalValorGeral)}\n`;
            resumo += '\n';
    ...
    adicionarResumoPDF(doc, yInicial, registros) {
        try {
            ...
            doc.text(`Saldo Banco: ${totais.totalBancoHoras}`, coluna2X, y + 5);
            doc.text(`Bônus 90%: ${totais.totalHorasExtras}`, coluna4X, y + 5);
...
console.log('📤 Exportador de Dados carregado - v2.0.0');
