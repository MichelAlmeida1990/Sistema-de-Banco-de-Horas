...
class BancoHorasApp {
    ...
    async salvarRegistro(dados) {
        try {
            // Obter valor da hora atual da configuração
            ...
            // Incluir valor da hora no registro
            const registro = {
                ...dados,
            ...
            // Salvar no Firestore
            const docRef = await addDoc(collection(this.db, 'registros'), registro);
            
            // Atualizar lista local
            this.registros.push({ id: docRef.id, ...registro });
    ...
    renderizarRegistros() {
        try {
            ...
            registrosOrdenados.forEach(registro => {
                ...
                const tipoPlantao = registro.feriado ? 'Feriado' : 
                                  (registro.fimDeSemana || this.calculadora.ehFimDeSemana(registro.data)) ? 'Fim de Semana' : 
                                  'Normal';

                html += `
                    <div class="registro-item grid grid-cols-11 gap-2 items-center py-2 border-b">
                        <div class="text-blue-600">
                            ${this.formatarData(registro.data)}
                            <div class="text-xs text-gray-500">${this.obterDiaSemana(registro.data)}</div>
                        </div>
                        ...
                        <div class="text-amber-600">
                            ${registro.pausa || 0}min
                        </div>
                        
                        <div>
                            <span class="tipo-plantao ${this.obterClasseTipo(tipoPlantao)}">
                                ${tipoPlantao}
                            </span>
                        </div>
                        ...
                        <div class="text-purple-600">
                            ${this.formatarSaldo(calculo.saldoHoras)}
                        </div>
                        
                        <div class="text-blue-600">
                            ${calculo.horasExtrasRemuneradas > 0 ? calculo.horasExtrasRemuneradas.toFixed(1) + 'h' : '-'}
                        </div>
                        
                        <div class="text-indigo-600">
                            ${calculo.bancoHoras > 0 ? calculo.bancoHoras.toFixed(1) + 'h' : '-'}
                        </div>
            ...
            container.innerHTML = html || '<p class="text-gray-500 text-center py-4">Nenhum registro encontrado</p>';
            
            console.log('🎨 Interface atualizada com', registrosOrdenados.length, 'registros');

        } catch (error) {
            console.error('❌ Erro ao renderizar registros:', error);
            this.mostrarNotificacao('❌ Erro ao exibir registros', 'error');
        }
    }
