...
class BancoHorasApp {
    constructor(uid) {
        try {
            ...
            throw new Error('Falha ao construir aplicação: ' + error.message);
        }
    }
    ...
    // ✅ CORRIGIDO - Inicializar aplicação
    ...
    async salvarRegistro() {
        try {
            ...
            if (!this.uid) {
                this.mostrarNotificacao('❌ Você precisa estar logado para salvar registros', 'error');
                return;
            }
            ...
            // 3. Obter valor da hora atual da configuração
            const configuracao = JSON.parse(localStorage.getItem('configuracaoFinanceira') || '{}');
            const valorHora = parseFloat(configuracao.valorHora);

            if (!valorHora || valorHora <= 0) {
                this.mostrarNotificacao('❌ Configure o valor da hora antes de salvar o registro', 'error');
                return;
            }
            ...
            // 5. Criar registro com dados limpos e valor da hora
            const registro = {
                ...
                fimDeSemana: dados.fimDeSemana || false,
                feriado: dados.feriado || false,
                valorHora: valorHora, // Salvar valor da hora no registro
            ...
            // 8. Limpar formulário e atualizar interface
            ...
            this.mostrarNotificacao('✅ Registro salvo com sucesso!', 'success');
            console.log('✅ Registro salvo:', registro);

        } catch (error) {
            console.error('❌ Erro ao salvar registro:', error);
            this.mostrarNotificacao('❌ Erro ao salvar registro', 'error');
        }
    }

    // ✅ CORRIGIDO - Coletar dados do formulário
    coletarDadosFormulario() {
        try {
            ...
            const fimDeSemana = document.getElementById('fimSemana')?.checked;
            ...
            return {
                ...
                fimDeSemana: Boolean(fimDeSemana),
                usarBancoHoras: Boolean(usarBancoHoras),
        ...
        try {
            ...
            registrosOrdenados.forEach(registro => {
                try {
                    ...
                    // Calcular valores
                    const calculo = this.calculadora.calcularValorRegistro(
                        registro,
                        registro.valorHora || this.configuracao.valorHora
                    );
    ...
    resetarDados() {
        try {
            ...
            // Resetar configuração financeira
            const configPadrao = {
                valorHora: 38.00,
                dataAtualizacao: new Date().toISOString()
            };
            localStorage.setItem('configuracaoFinanceira', JSON.stringify(configPadrao));
            ...
            // Atualizar campo valor hora
            const campoValorHora = document.getElementById('valorHora');
            if (campoValorHora) {
                campoValorHora.value = configPadrao.valorHora.toFixed(2);
                this.atualizarValorComBonus();
            }
            
            console.log('🔄 Dados resetados com sucesso');
            this.mostrarNotificacao('✅ Todos os dados foram resetados!', 'success');
