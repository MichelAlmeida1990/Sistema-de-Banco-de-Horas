...
class RegistroPlantao {
    ...
    async carregarRegistros() {
        try {
            ...
            // Corrigir registros antigos que não têm valor da hora
            await this.corrigirRegistrosAntigos();
    ...
    async corrigirRegistrosAntigos() {
        try {
            const valorHoraPadrao = this.configuracao.getValorHora();
            let registrosCorrigidos = 0;

            for (const registro of this.registros) {
                // Se o registro não tem valor da hora ou é inválido
                if (!registro.valorHora || registro.valorHora <= 0) {
                    console.log(`🔧 Corrigindo registro ${registro.id} - adicionando valor da hora: R$ ${valorHoraPadrao}`);
                    
                    // Atualizar o registro
                    registro.valorHora = valorHoraPadrao;
                    ...
                    try {
                        ...
                        console.error(`❌ Erro ao corrigir registro ${registro.id}:`, error);
                    }
                }
            }

            if (registrosCorrigidos > 0) {
                console.log(`✅ ${registrosCorrigidos} registros antigos corrigidos`);
                this.mostrarInfo(`${registrosCorrigidos} registros antigos foram atualizados com o valor da hora atual`);
            }
        } catch (error) {
            console.error('❌ Erro ao corrigir registros antigos:', error);
        }
    }
    ...
    renderizarRegistros() {
        ...
        if (!container) {
            console.error('❌ Container listaRegistros não encontrado');
            return;
        }
