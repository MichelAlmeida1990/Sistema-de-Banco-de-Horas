...
class CalculadoraBancoHoras {
    constructor() {
        ...
        this.BONUS_FIM_SEMANA = 1.90; // 90% de bônus
        
        console.log('✅ CalculadoraBancoHoras inicializada');
    }
    ...
    calcularValorRegistro(registro, valorHoraAtual) {
        try {
            ...
            const valorHoraCalculado = temBonus ? valorHora * 1.9 : valorHora; // 90% adicional (multiplicador 1.9)
            ...
            const resultado = {
                ...
                multiplicadorBonus: temBonus ? 1.9 : 1.0
            };
    ...
    // ✅ CORRIGIDO - Calcular horas trabalhadas
    calcularHorasTrabalhadas(entrada, saida) {
        try {
            ...
            const minEntrada = this.converterParaMinutos(entrada);
            const minSaida = this.converterParaMinutos(saida);
            
            // Calcular diferença
            let minutosTrabalhados = minSaida - minEntrada;
            
            // Se virou o dia (saída menor que entrada)
            if (minutosTrabalhados < 0) {
                minutosTrabalhados += 24 * 60; // Adicionar 24h em minutos
            }
            
            // Converter para horas com uma casa decimal
            const horasTrabalhadas = Math.round((minutosTrabalhados / 60) * 10) / 10;
            
            return horasTrabalhadas;
            
        } catch (error) {
            console.error('❌ Erro ao calcular horas trabalhadas:', error);
            throw new Error('Erro ao calcular horas trabalhadas: ' + error.message);
        }
    }

    // ✅ CORRIGIDO - Converter horário para minutos
    converterParaMinutos(horario) {
        try {
            ...
            console.error('❌ Erro ao converter para minutos:', error);
            throw new Error('Erro ao converter horário: ' + error.message);
        }
    }

    // ✅ CORRIGIDO - Verificar se é fim de semana
    ehFimDeSemana(data) {
        try {
            ...
            if (ehFimDeSemana) {
                console.log(`📅 Fim de semana detectado: ${data} (${diaSemana === 0 ? 'Domingo' : 'Sábado'})`);
            }
