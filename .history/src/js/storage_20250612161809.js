...
// Removido import/export para uso global no navegador
...
// --- FIRESTORE STORAGE PARA USO GLOBAL ---
class GerenciadorFirestore {
  constructor() {
    this.versao = '5.0.0-firebase';
  }
  getUserRegistrosRef(uid) {
    return firebase.firestore().collection('usuarios').doc(uid).collection('registros');
  }
  async carregarRegistros(uid) {
    const registrosRef = this.getUserRegistrosRef(uid);
    const snapshot = await registrosRef.get();
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  }
  async adicionarRegistro(uid, registro) {
    const registrosRef = this.getUserRegistrosRef(uid);
    const docRef = await registrosRef.add({
      ...registro,
      criadoEm: new Date().toISOString(),
      atualizadoEm: new Date().toISOString()
    });
  ...
  async atualizarRegistro(uid, id, dadosAtualizados) {
    const registroDoc = this.getUserRegistrosRef(uid).doc(id);
    await registroDoc.update({
      ...dadosAtualizados,
      atualizadoEm: new Date().toISOString()
    });
    return true;
  }
  async removerRegistro(uid, id) {
    const registroDoc = this.getUserRegistrosRef(uid).doc(id);
    await registroDoc.delete();
    return true;
  }
  async buscarRegistros(uid, criterios = {}) {
    let registrosRef = this.getUserRegistrosRef(uid);
    if (criterios.data) {
      registrosRef = registrosRef.where('data', '==', criterios.data);
    }
    const snapshot = await registrosRef.get();
    return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  }
...
// Remover export e expor globalmente
window.GerenciadorFirestore = GerenciadorFirestore;
