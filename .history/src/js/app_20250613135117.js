...
class BancoHorasApp {
    ...
    async salvarRegistro() {
        try {
            ...
            const registro = {
                ...
                uid: this.uid // Adicionar uid ao registro
            };
            console.log('📋 Registro montado:', registro);
            ...
            try {
                ...
                // Atualizar interface
                await this.carregarDados(); // Recarregar dados do Firestore
                this.renderizarRegistros();
                this.atualizarTotais();
                this.atualizarContadores();
    ...
    async carregarDados() {
        try {
            ...
            const registros = await this.storage.listarRegistros();
            
            // Filtrar registros do usuário atual
            this.registros = registros.filter(r => r.uid === this.uid);
    ...
    async excluirRegistro(id) {
        await this.storage.removerRegistro(this.uid, id);
        await this.carregarDados();
        this.renderizarRegistros();
        this.atualizarTotais();
        this.atualizarContadores();
...
// Inicialização aguardando autenticação
let app = null;
...
// Atualiza funções globais para async
