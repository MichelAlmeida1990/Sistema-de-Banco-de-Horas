...
class GerenciadorStorage {
    ...
    class GerenciadorFirestore {
      ...
      // Helper para obter referência da coleção do usuário
      ...
      async atualizarRegistro(uid, id, dadosAtualizados) {
        const registroDoc = doc(db, 'usuarios', uid, 'registros', id);
        await updateDoc(registroDoc, {
          ...dadosAtualizados,
          atualizadoEm: new Date().toISOString()
        });
        return true;
      ...
      // Buscar registros por critérios (exemplo: data)
      async buscarRegistros(uid, criterios = {}) {
        let registrosRef = this.getUserRegistrosRef(uid);
        let q = registrosRef;
        // Exemplo: buscar por data exata
        if (criterios.data) {
          q = query(registrosRef, where('data', '==', criterios.data));
        }
